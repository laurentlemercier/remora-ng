...
This document describes the technical architecture of the Remora custom component for Home Assistant custom component for Home Assistant.
...
│   ├── __init__.py          # Exports RemoraDataUpdateCoordinator
...
│   ├── __init__.py          # Exports RemoraEntity
...
- `base.py` - Main coordinator class (`RemoraDataUpdateCoordinator`)
...
**Key class:** `RemoraDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `RemoraConfigFlowHandler` (main flow)
- `RemoraOptionsFlow` (options)
...
**Key class:** `RemoraEntity` (in `entity/base.py`)
...
2. `RemoraEntity` for common functionality
